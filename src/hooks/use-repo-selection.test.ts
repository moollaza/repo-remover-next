...
describe("useRepoSelection", () => {
  ...
  describe("isRepoDisabled", () => {
    ...
    it("returns true for locked repos regardless of action", () => {
      const lockedRepo = createMockRepo({ isLocked: true });
      const { result } = renderHook(() => useRepoSelection(defaultProps));

      // Archive action (default)
      expect(result.current.isRepoDisabled(lockedRepo)).toBe(true);
  ...
  describe("handleRowSelect", () => {
    ...
    it("does not select disabled repos", () => {
  ...
  describe("handleSelectAll", () => {
    it("selects all non-disabled repos from filteredRepos", () => {
      const repos = [
        createMockRepo({
          id: "repo-1",
          isArchived: false,
    ...
    it("excludes locked repos from select-all", () => {
      const repos = [
        createMockRepo({ id: "repo-1", key: "repo-1" }),
        createMockRepo({ id: "repo-2", isLocked: true, key: "repo-2" }),
      ];
      const { result } = renderHook(() =>
        useRepoSelection({
          filteredRepos: repos,
      ...
      const selected = result.current.selectedRepoKeys as Set<string>;
      expect(selected.has("repo-1")).toBe(true);
      expect(selected.has("repo-2")).toBe(false);
    });
  });
  ...
  describe("disabledKeys", () => {
    ...
    it("includes locked repos regardless of action", () => {
      const repos = [
        createMockRepo({
          id: "locked-1",
          isLocked: true,
      ...
      expect(result.current.disabledKeys.has("locked-1")).toBe(true);

      // Switch to delete — still disabled
      act(() => {
        result.current.handleRepoActionChange(new Set(["delete"]));
      });
